...
import { api, type SparklineEntry, type SparklineWindow } from "@/lib/api";

/**
 * Batched sparkline history for a list of token cards. Every visible mint is
 * fetched in ONE request (the server resolves the best real source + caches per
 * mint), so a 30-card list costs a single round-trip rather than one call per card.
 *
 ...
 * when the set actually changes. Returns a lookup of resolved entries:
 *   - `{ points: number[], source }` → draw the real line
 *   - `{ points: null, source: null }` → no real data; the card draws an L6 placeholder
 *   - `undefined` → still loading (render loading placeholder)
 */
export function useSparklines(
  mints: string[],
  window: SparklineWindow = "24h",
): Record<string, SparklineEntry | undefined> {
