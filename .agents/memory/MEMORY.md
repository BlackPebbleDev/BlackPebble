...
- [DexScreener pair selection](dexscreener-pair-selection.md) — pick pools by TRUSTED quote (wSOL/USDC/USDT) first, not raw liquidity; junk-quote pools inflate price/MC/24h (USELESS/FARTCOIN +520k%). Keep isBetterPair as the one selection primitive across all 4 per-mint paths.
